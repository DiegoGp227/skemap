  ...
  return (
    <div className="max-w-5xl mx-auto px-8 py-16 grid grid-cols-[280px_1fr] gap-16">
      ...
      <div className="flex flex-col gap-5">
        ...
        <div>
          <h1 className="text-fg font-bold text-xl leading-tight">{user.name}</h1>
...
<Row icon={<User className="w-3.5 h-3.5" />} label="Name" value={user.name} />
            <Row icon={<AtSign className="w-3.5 h-3.5" />} label="Username" value={`@${user.username}`} />
            <Row icon={<Mail className="w-3.5 h-3.5" />} label="Email" value={user.email} />
            <Row icon={<Calendar className="w-3.5 h-3.5" />} label="Member since" value={joined} />
            <Row icon={<Clock className="w-3.5 h-3.5" />} label="Last updated" value={updated} />
          </div>
        </div>
      </div>
...
function Row({ icon, label, value, mono }: { icon: React.ReactNode; label: string; value: string; mono?: boolean }) {
  return (
    <div className="flex items-center justify-between py-3.5 gap-4">
      ...
      <span className={`text-sm text-fg-secondary font-medium ${mono ? "font-mono" : ""}`}>{value}</span>
    </div>
  );
}
